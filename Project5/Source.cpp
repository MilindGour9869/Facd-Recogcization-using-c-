...
void detectAndDisplay(Mat frame)
{
	...
	Mat gray;
	string text;
	stringstream sstm;  // used for formatting/parsing/converting a string to number/char etc
	...
	{
		roi_c.x = faces[ic].x;   // roi = region of interest 
		roi_c.y = faces[ic].y;
		...
		stringstream ssfn;
		filename = "D:\\open cv images data\\img\\";
...
void addFace()
{
	...
	face_cascade.load("D:\\AAdam\\build\\install\\etc\\haarcascades\\haarcascade_frontalface_alt.xml");
	if (!face_cascade.load("D:\\AAdam\\build\\install\\etc\\haarcascades\\haarcascade_frontalface_alt.xml"))
	{
		cout << "error in harrarcascdes" << endl;
		return;
	}
...
void eigenFaceTrainer() {
	...
	model->save("D:\\open cv images data\\eigenface.yml");
...
void  FaceRecognition() {
	...
	//loads pre-trained data sets
	Ptr<FaceRecognizer>  model = FisherFaceRecognizer::create();
	model->read("D:\\open cv images data\\eigenface.yml");

	Mat testSample = imread("D:\\open cv images data\\img\\10.jpg", 0);
	...
	// opening a new window for the camera frame , showings resuts also ...
...
{
	int i;
	cout << " Enter 1 for Add Face\n " <<endl;
	cout << " Enter  2  for  Face recognization\n " << endl;
	cin >> i;
